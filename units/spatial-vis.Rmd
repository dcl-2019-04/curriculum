...
`nc_74` contains data on deaths from Sudden Infant Death Syndrome (SIDS) in 1974 for counties in North Carolina.
...
When you include multiple geospatial layers, ggplot2 will ensure that they all have a common coordinate reference system (CRS) so that it makes sense to overlay them.
...
* You can also use `coord_sf()` to override the projection in the data. If you don't specify the `crs` argument to `coord_sf()`, if defaults to the CRS given in the data.
    ...
    If you're visualizing the entire US, it's better to use the US Albers projection. Albers is an equal-area projection, which means it accurately reflects the areas of geographic regions. The ESPG code of US Albers is 102003.
